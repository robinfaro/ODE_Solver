...
{
    if (coefficient.size() != initial_condition.cols() + 1)
        throw std::invalid_argument("Coefficient vector must have the same size as the number of steps in the initial condition matrix plus one.");
    if (coeff_type == ALPHA)
    {
